...
import { type Provider, type Review } from "@shared/schema";
import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Avatar, AvatarImage, AvatarFallback } from "@/components/ui/avatar";
import { useToast } from "@/hooks/use-toast";
import { Star, Shield, Award, Clock, Leaf, Home as HomeIcon, Users, Check, MapPin, Crown, Calendar, LoaderPinwheel } from "lucide-react";
...
export default function Home() {
  ...
  const reviews = provider ? [
    ...
    },
    {
      ...
      customerInitials: "MC",
      rating: 5,
      comment: "Excellent service! Very thorough cleaning and uses eco-friendly products. Great communication and flexible scheduling. Highly recommend!",
    ...
    },
    {
      ...
      customerInitials: "LT",
      rating: 4,
      comment: "Professional and trustworthy cleaner. Always leaves my home spotless and smelling fresh. Will definitely book again!",
    ...
    <div className="bg-background font-lato text-foreground min-h-screen">
      ...
      <header className="bg-primary text-primary-foreground shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <h1 className="font-poppins font-bold text-xl" data-testid="header-title">CleanConnect</h1>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        ...
        <Card className="bg-card rounded-lg shadow-lg overflow-hidden mb-6" data-testid="provider-profile">
            ...
            <div className="flex items-start space-x-4">
              <div className="relative">
                <Avatar className="w-20 h-20 border-4 border-white shadow-lg" data-testid="provider-avatar">
                  <AvatarImage src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-4.0.3&auto=format&fit=crop&w=200&h=200" alt={`${provider.name} profile`} />
              ...
              <div className="flex-1">
                <h2 className="font-poppins font-bold text-xl mb-1" data-testid="provider-name">{provider.name}</h2>
          ...
          {/* Provider Bio */}
          <div className="p-6">
            <h3 className="font-poppins font-semibold text-lg mb-3 soft-teal">About {provider.name.split(' ')[0]}</h3>
            <p className="text-muted-foreground leading-relaxed mb-4" data-testid="provider-bio">
              {provider.bio || "Professional house cleaner with years of experience providing quality cleaning services."}
            </p>
          ...
          {reviewsLoading ? (
            ...
            <div className="space-y-4" data-testid="reviews-container">
              {sortedReviews.map((review, index) => (
                <div key={review.id} className={`border-b border-border pb-4 ${index === sortedReviews.length - 1 ? 'border-b-0' : ''}`} data-testid={`review-${review.id}`}>
                  <div className="flex items-start space-x-3">
                    <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center">
                      <span className="text-primary font-medium text-sm">{review.customerInitials}</span>
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-medium text-sm" data-testid={`review-customer-${review.id}`}>{review.customerName}</span>
        ...
        <Card className="bg-card rounded-lg shadow-lg p-6" data-testid="booking-section">
          <div className="text-center">
            <h3 className="font-poppins font-semibold text-lg mb-2">Ready to Book {provider.name.split(' ')[0]}?</h3>
            <p className="text-muted-foreground mb-4">Join our satisfied customers with professional cleaning services</p>
